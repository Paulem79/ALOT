...
public class Biome_1_18_2 {
    ...
    public static Biome retrieve(){
        ...
        Registry<Biome> registrywritable = dedicatedserver.registryAccess().ownedRegistryOrThrow(Registry.BIOME_REGISTRY);
        return registrywritable.get(oldKey);
    }

    public static void newBiome() throws NoSuchFieldException, IllegalAccessException {
        Biome forestbiome = retrieve();
        Biome.BiomeBuilder newBiome = new Biome.BiomeBuilder();
        Field biomeSettingGenField = Biome.class.getDeclaredField("l");
        biomeSettingGenField.setAccessible(true);
        newBiome.biomeCategory((Biome.BiomeCategory) biomeSettingGenField.get(forestbiome));
        ...
        ((WritableRegistry<Biome>) dedicatedserver.registryAccess().ownedRegistryOrThrow(Registry.BIOME_REGISTRY)).register(getKey(), newBiome.build(), Lifecycle.stable());
    }
    ...
    private void refreshChunksForAll(Chunk chunk) {
        ...
        //level.getChunkSource().chunkMap.resendChunk(craftChunk.getHandle());
    }
}
